...
alter table exp_pre_dist drop column select_site;
alter table exp_pre_dist add column select_site bool default false;
update exp_pre_dist t1 set select_site=true from
         (select distinct on (pdbid,exp_residueid) pdbid,exp_residueid,pre_id,dist from (select * from exp_pre_dist where dist <= 2.5 ) a
                order by pdbid,exp_residueid,dist) t2
         where t1.pdbid=t2.pdbid and t1.exp_residueid=t2.exp_residueid and t1.pre_id=t2.pre_id;
...
update zinc_predict_site234_2 t1 set bench=t2.bench from
        (select * from exp_pre_dist where select_site is true and pre_id in (select distinct id from zinc_predict_site234_2 where selected_site is true) )t2 where t1.id=t2.pre_id;


alter table exp_site234 add column id smallint default -1;
update exp_site234 t1 set id=t2.pre_id from 
	(select * from exp_pre_dist where select_site is true and pre_id in (select distinct id from zinc_predict_site234_2 where selected_site is true) ) t2 
